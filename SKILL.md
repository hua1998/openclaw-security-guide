---
name: "openclaw-security-assessment-v2"
...
### 12. MAESTRO 基础模型层 (权重7%)
- `agents.bootstrap.protect_prompts` - 系统提示保护
- `model.useKeychain` - 模型 API 密钥安全存储
- `session.compaction.enabled` - 会话压缩防止越狱

### 13. MAESTRO 数据操作层 (权重7%)
- `pairing.secureStorage` - 凭证安全存储
- `state.restrictPermissions` - 状态目录权限限制
- `security.dataEncryption.enabled` - 数据加密启用

### 14. MAESTRO 工具使用层 (权重7%)
- `tools.permissions.enabled` - 工具权限控制
- `tools.deny` - 危险工具禁用

### 15. MAESTRO 网络交互层 (权重7%)
- `network.mode` - 网络策略模式
- `network.allow_external` - 外部网络访问控制

### 16. MAESTRO 沙箱逃逸层 (权重7%)
- `agents.defaults.sandbox.mode` - 沙箱隔离模式
- `agents.defaults.sandbox.resource_limits.enabled` - 沙箱资源限制

### 17. MAESTRO 持久化层 (权重7%)
- `config.restrictPermissions` - 配置文件权限限制
- `credentials.secureStorage` - 凭证安全存储

### 18. MAESTRO 横向移动层 (权重7%)
- `channels.*.isolate` - 通道隔离
...
```
输入接收 → 配置解析 → 18维度检查 → 风险评级 → 生成报告 → 提供加固方案 → 合规映射
```
...
| 场景特定安全 | XX/100 | 5% | XX |
| MAESTRO 基础模型层 | XX/100 | 7% | XX |
| MAESTRO 数据操作层 | XX/100 | 7% | XX |
| MAESTRO 工具使用层 | XX/100 | 7% | XX |
| MAESTRO 网络交互层 | XX/100 | 7% | XX |
| MAESTRO 沙箱逃逸层 | XX/100 | 7% | XX |
| MAESTRO 持久化层 | XX/100 | 7% | XX |
| MAESTRO 横向移动层 | XX/100 | 7% | XX |
| **综合评分** | **XX/100** | 100% | **XX** |
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v2.2 | 2026-03-13 | 增强版：18维度评分、MAESTRO框架集成、7层智能AI威胁模型检测 |
